...
function js(done) {
    webpack(webpackConfig, function (err, stats) {
        ...
        if (stats.hasErrors()) {
            console.log('Ошибка сборки Webpack', stats.errors, stats);
        }
...
function observe(done) {
    ...
    watch('src/**/*.+(svg|png)').on('change', series(img, reload));
...
exports.default = series(parallel(html, css, js, img), serve, observe);